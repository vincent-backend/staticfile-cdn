...
const api_libraries = settings.api_root + "libraries/";
...
export const getPlatformData = async () => {
  try {
    const res = await fetch(settings.api_statistics + "platforms");
    const data = await res.json();
    const rslt = true;
    return { rslt, data };
  }
...
export const getBrowsersData = async () => {
  try {
    const res = await fetch(settings.api_statistics + "browsers");
    const data = await res.json();
    const rslt = true;
    return { rslt, data };
  }
...
export const getPackagesData = async () => {
  try {
    const res = await fetch(settings.api_statistics + "packages");
    const data = await res.json();
    const rslt = true;
    return { rslt, data };
  }
